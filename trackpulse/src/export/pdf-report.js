 ...
 *
 * Report includes:
 * - Header with TrackPulse branding (or custom logo for Agency white-label)
...
export async function generateAuditReport(state, options = {}) {
  ...
  if (options.whiteLabelLogo && options.agencyName) {
    ...
    doc.text('TrackPulse — Tracking Audit Report', margin, 22);
  }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(...grayText);
    doc.setFontSize(8);
    doc.text(options.whiteLabelFooter || 'Generated by TrackPulse', margin, 290);
  ...
  const filename = `trackpulse-audit-${hostname}-${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
...
export async function generateFunnelPDFReport(report, options = {}) {
  ...
  doc.setFontSize(20);
  doc.setFont('helvetica', 'bold');
  doc.text(options.agencyName || 'TrackPulse — Funnel Audit Report', margin, 22);
  ...
  const filename = `trackpulse-funnel-${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
